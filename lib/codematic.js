...
module.exports = function(options) {
   ...
   */
  var runRuleCurry = function(key, before) {
  ...
  this.runStartup = runRuleCurry('_onStartup',
    // Before
    function() {
  ...
  this.runShutdown = runRuleCurry('_onShutdown',
    // Before
    function() {
   ...
   * @return {string}
   */
  this.insertVars = function(subject, store, prefix) {
    ...
    if (typeof subject == 'string') {
      Object.keys(store).forEach(function(key) {
        if ((typeof store[key] == 'string') || (typeof store[key] == 'number')) {
          subject = subject.replace('%' + prefix + key + '%', store[key]);
        } else if (typeof store[key] == 'object' && store[key] !== null) {
          subject = this.insertVars(subject, store[key], key + '.');
        }
      }.bind(this));
    }
