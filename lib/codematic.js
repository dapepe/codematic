...
module.exports = function(options) {
  ...
  this.run = function(code, callback) {
    for (var i = 0 ; i < RULES.length ; i++) {
      if (this.checkRule(RULES[i], code)) {
        if (typeof this._log.file == 'string') {
          fs.appendFile(this._log.file, (new Date()).toLocaleDateString() + ': ' + RULES[i].name + ': ' + code, function(err) {
            if (err) {
              console.log('Error writing log file', this._log, err);
            }
          }.bind(this));
        }
        this.runRule(RULES[i], code, callback);
        return;
      }
    }

    console.log('No rule found for code ', code);
    callback();
  }
