...
module.exports = {
  ...
  'request': function(action, callback) {
    ...
    var r = request(callparams, function(err, response, body) {
      this.store(
        typeof action['var'] == 'undefined' && action['var'] != '' ? 'BODY' : action['var'],
        typeof action.decode_json !== 'undefined' && action.decode_json ? JSON.parse(body) : body
      );
      ...
      // Resonse objects are simply too large
      /*
